...
class NodeLoad {
	...
	init(callback) {
		...
		var xtrawebscriptloader = modulescriptloader.getChildLoader('@primusmoney/xtrawebconfig');
		...
		xtrawebscriptloader.load_scripts(function () {
			...
			_nodeobject.loadModule('dapps', xtra_webmodulescriptloader, function() {
				rootscriptloader.signalEvent('on_@primusmoney_xtra_web_module_ready');
				
			});
		});
		
		// end of modules load
		rootscriptloader.registerEventListener('on_@primusmoney_xtra_web_module_ready', function(eventname) {
			if (callback)
				callback(null, self);
		});
